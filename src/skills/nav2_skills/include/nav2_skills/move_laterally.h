...
#ifndef MOVE_LATERALLY_SKILL
#define MOVE_LATERALLY_SKILL
#pragma once
...
{
class MoveLaterally : public BT::RosActionNode<nav2_msgs::action::DriveOnHeading>
{
public:
  MoveLaterally(
  ...
  bool setGoal(Goal & goal)
  {
    ...
    RCLCPP_INFO_STREAM(
      node_->get_logger(), "Moving of " << distance << " on y direction at " << speed);
    goal.target.y = distance;
  ...
  static BT::PortsList providedPorts()
  {
    return providedBasicPorts({
      BT::InputPort<double>("distance", "Distance to travel in y."),
      BT::InputPort<double>("speed", "Speed to command."),
      BT::InputPort<double>("time_allowance", "Time that is allowed to move in seconds."),
    });
  }
};
}  // namespace nav2_skills
#endif  // MOVE_LATERALLY_SKILL
