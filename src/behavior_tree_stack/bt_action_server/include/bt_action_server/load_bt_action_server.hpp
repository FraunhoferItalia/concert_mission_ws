...
#ifndef LoadBTActionServer_H
#define LoadBTActionServer_H

#pragma once

#include <bt_executor/bt_executor.h>

#include <behavior_tree_msgs/action/load_behavior_tree.hpp>
...
class LoadBTActionServer
{
public:
  using ActionType = behavior_tree_msgs::action::LoadBehaviorTree;
  ...
  /**
   * @brief Construct a new action server class that exposes the ability to load a tree from a file
   * once the action is called.
   *
   * @param tree the pointer to the executor.
   * @param node the pointer to the ROS2 node.
   */
  explicit LoadBTActionServer(BTExecutorPtr tree, std::shared_ptr<rclcpp::Node> node);

  ~LoadBTActionServer(void) {}
